...
export default async function handler(
  ...
  const { text, color = 'rgb(0,0,0,0.04)' } = req.query
...
function mountBodySvg({
  ...
  const header = `<text x='${propsX}' y='${propsY}' fill='${color}' font-size='4rem' font-weight='600' font-family='Roobert,Helvetica Neue,Helvetica,Arial,sans-serif'>`
  const footer = '</text>'
